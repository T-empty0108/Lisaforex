# ═══════════════════════════════════════════════════════════════════════════════
# LISA FOREX - TELEGRAM HTML BOT (Telethon)
# ═══════════════════════════════════════════════════════════════════════════════
...
APISED_KEY = os.getenv("APISED_KEY", "")
...
def parse_yesterday_report(text, target_date_str):
    if "Tổng hợp lệnh XAU ngày" not in text or target_date_str not in text:
        return None
    try:
        # Loại bỏ Markdown formatting trước khi parse
        clean_text = text.replace("**", "").replace("__", "")
        
        # Format: 🟢 ✅ TP: 5 (Tổng +276.0 pip)
...
class LisaForexBot:
    ...
    async def load_signal_history(self):
        print(f"📜 Loading {HISTORY_LIMIT} messages từ Signal group...")
        try:
            ...
            print(f"❌ Lỗi load signal history: {e}")
    ...
    async def load_yesterday_report(self):
        ...
        try:
            ...
            print(f"🔍 Tìm report ngày: {target_date_str}")
            
            entity = await self.client.get_entity(PeerChannel(REPORT_GROUP_ID))
            
            # Thu thập TẤT CẢ reports của ngày target
            all_reports = []
            ...
            # Chọn report có NET cao nhất (hoặc có nhiều lệnh nhất)
            if all_reports:
                ...
                print(f"⚠️ Không tìm thấy report ngày {target_date_str}")
                self.report = None
        except Exception as e:
            print(f"❌ Lỗi load report: {e}")
    ...
    async def run(self):
        print("=" * 60)
        print("🦊 LISA FOREX - Telegram HTML Bot")
        print("=" * 60)
        ...
        if not API_ID or not API_HASH:
            print("❌ Chưa điền API_ID và API_HASH trong file .env")
            return
        ...
        print("=" * 60)
        print("✅ Bot sẵn sàng! Đang lắng nghe messages...")
